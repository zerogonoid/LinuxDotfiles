...
myStartupHook = do
  ...
  spawnOnce "xwallpaper --zoom ~/.config/wall/master"
...
myKeys c =
  ...
  , ("M-S-<Return>", addName "simple dmenu"            $ spawn ("dmenu_run -i -p \"Run: \"")) -- Dmenu
  , ("M-e", addName "Launch web browser"       $ spawn (myEmacs))
  ...
  ^++^ subKeys "Scratchpads"
  [ ("M-i", addName "Toggle scratchpad terminal"   $ namedScratchpadAction myScratchPads "terminal")
